...
export async function delegateTask(taskDescription, message) {
  ...
  return pollToCompletion(taskId);
}

// Reply to a receiver's clarifying question, then wait for the final result.
// (SYNC-1 Tier-1 — paired with delegateTask, which returns a NEEDS INPUT prompt
// when the receiver asks. The calling agent answers with `ammunity answer`.)
export async function answerTask(taskId, answer) {
  requireCredentials();
  const res = await fetch(`${COORDINATOR_URL}/tasks/${taskId}/answer`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-Agent-Key": SELF_AGENT_KEY,
  ...
  if (!res.ok) {
    ...
    throw new Error(`Answer failed: ${res.status} ${body}`);
  }
  return pollToCompletion(taskId);
}

// Poll a task to a terminal state OR to 'needs_input'. On needs_input we stop and
// surface the question instead of blocking — the CLI sender is fire-and-exit by
// design, so the calling agent answers with a follow-up `ammunity answer` call.
async function pollToCompletion(taskId) {
  const deadline = Date.now() + MAX_WAIT_MS;
  let state;
  while (Date.now() < deadline) {
    await sleep(POLL_INTERVAL_MS);
    state = await pollStatus(taskId);
    if (state.status === "needs_input") {
      return formatNeedsInput(taskId, state.pending_question);
    }
    if (TERMINAL_STATUSES.has(state.status)) {
      return formatTerminal(state);
    }
  }
  throw new Error(
...
function formatNeedsInput(taskId, question) {
  return (
    `NEEDS INPUT — the agent needs one detail before it can finish.\n` +
    `QUESTION: ${question || "(no question text provided)"}\n` +
    `To answer, run: ammunity answer ${taskId} "your answer here"`
  );
}

function formatTerminal(state) {
  switch (state.status) {
    case "completed":
...
async function main() {
  ...
  if (command === "answer") {
    // ammunity answer <task_id> "<reply>"   (reply to a receiver's question)
    const rest = args.slice(1);
    const taskId = rest[0];
    const answer = rest.slice(1).join(" ").trim();
    if (!taskId || !answer) {
      console.error('Usage: ammunity answer <task_id> "your answer"');
      process.exit(2);
    }
    try {
      const result = await answerTask(taskId, answer);
      process.stdout.write(result);
      process.exit(0);
    } catch (e) {
      console.error(e.message);
  ...
  console.log('Available commands: delegate "TASK" "MSG", answer <task_id> "REPLY", discover');
}
